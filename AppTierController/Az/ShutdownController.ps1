    ...
    for ($i = $MaxTiers; $i -gt 0; $i--)
    {
        ...
        {           
            try
            {
                ...
                #$result = Start-AzAutomationRunbook -AutomationAccountName $AutomationAccountName -Name $ChildRunbookName -ResourceGroupName $ResourceGroupName -Parameters $params -Wait -ErrorAction SilentlyContinue
                $result = Start-AzAutomationRunbook -AutomationAccountName $AutomationAccountName -Name $ChildRunbookName -ResourceGroupName $AAResourceGroupName -Parameters $params -Wait -ErrorAction SilentlyContinue
                Write-Output $result
